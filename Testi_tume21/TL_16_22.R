...
## Load data
esko_fonol = read_xlsx("C:/Users/03248355/Work Folders/Data/LTA1_SV.xlsx", sheet = 3)

## Create variables to indicate whether each item was correctly answered
...
# Create a variable to indicate total correctly answered items
esko_fonol  <- esko_fonol %>%
...
## Load data
esko_kirjain = read_xlsx("C:/Users/03248355/Work Folders/Data/LTA1_SV.xlsx", sheet = 4)
...
## Load data
esko_lukutaito = read_xlsx("C:/Users/03248355/Work Folders/Data/LTA1_SV.xlsx", sheet = 5)

## Create a variable to indicate whether each item was correctly answered and a
## separate variable to indicate whether they were attempted. We also create a
## variable to indicate total items correctly answered and total items attempted
esko_lukutaito = esko_lukutaito %>%
  filter(IDCode != "admin@esko.fi") %>% # Drop test account
  group_by(IDHash, IDCode) %>%
  mutate(
    Q1_AnsC_lt = ifelse(PreOrd == 1 & AnsC == 1, 1, 0),
    Q2_AnsC_lt = ifelse(PreOrd == 2 & AnsC == 1, 1, 0),
    Q3_AnsC_lt = ifelse(PreOrd == 3 & AnsC == 1, 1, 0),
    Q4_AnsC_lt = ifelse(PreOrd == 4 & AnsC == 1, 1, 0),
    Q1_att_lt = ifelse(PreOrd == 1,1,0),
    Q2_att_lt = ifelse(PreOrd == 2,1,0),
    Q3_att_lt = ifelse(PreOrd == 3,1,0),
    Q4_att_lt = ifelse(PreOrd == 4,1,0)) %>%
  summarise(
...
## Create a separate dataframe to use for the correlation matrix
...
ggsave("16_22_kt.png", plot = p, width = 6, height = 4, dpi = 300)
p
...
ggsave("16_22_lt.png", plot = p, width = 6, height = 4, dpi = 300)
p
...
ggsave("16_22_fon.png", plot = p, width = 6, height = 4, dpi = 300)
p
...
## Compute the average score for each item in letter recognition
variable_names <- paste0("Q", 1:4, "_AnsC_lt")
